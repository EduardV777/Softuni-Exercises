...
The TV uses an HDMI cable to connect to a game console. Both the game console and TV connect to a router via an ethernet cable to access the internet.
...
class ConnectViaPower:
    def connect_device_to_power_outlet(self, device): pass



class Television(ConnectViaRCA, ConnectViaHDMI, ConnectViaPower):
...
class DVDPlayer(ConnectViaHDMI, ConnectViaPower):
...
class GameConsole(ConnectViaHDMI, ConnectViaEthernet, ConnectViaPower):
    def connect_to_tv(self, television):
        self.connect_to_device_via_hdmi_cable(television)
...
class Router(ConnectViaEthernet, ConnectViaPower):
    def connect_to_tv(self, television):
